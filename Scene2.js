...
class Scene2 extends Phaser.Scene {
  ...
  create() {
    ...
    this.scoreLabel = this.add.bitmapText(10, 5, "pixelFont", `SCORE:${scoreFormated}      Lives:${count}`, 16);
  ...
  pickPowerUp(player, powerUp) {
    ...
    var scoreFormated = this.zeroPad(this.score, 6);
    this.scoreLabel.text = `SCORE:${scoreFormated}            Lives:${count}`
  ...
  hurtPlayer(player, enemy) {
    ...
    var scoreFormated = this.zeroPad(this.score, 6);
    this.scoreLabel.text = `SCORE:${scoreFormated}         Lives:${count}`
    console.log(`ban con ${count} lan`);
    console.log('điểm của bạn là:' + this.score);

    // this.scoreLabel.text = "SCORE " + scoreFormated;
    // var scoreFormated = this.zeroPad(this.score, 6);
    if (count == 0) {
      this.scoreLabel.text = "gameover, bấm CTRL+R để chơi lại";
      ipcRenderer.send('score', this.score);
      this.score = 0;
  ...
  //khi bắn ra thì chạy file âm thanh để nó có tiếng chíu chíu
  shootBeam() {
